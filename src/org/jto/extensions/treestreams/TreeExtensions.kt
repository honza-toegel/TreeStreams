...
/**
 * BFS breadth-first-traversal -> whole levels
 */
fun <T, R, I> mapForestBfLevelsIndexed(rootNodes: Iterable<T>,
                                       initialIndex: I,
                                       children: (T) -> Iterable<T>,
                                       levelTransform: (index: I, levelNodes: Iterable<T>) -> List<R>,
                                       levelUpIndexTransform: (index: I, levelNodes: Iterable<T>) -> I): List<List<R>> {
            ...
            when (levelNodes.count()) {
                0 -> emptyList()
                    ...
                    listOf(levelTransform(index, levelNodes)) +
                            ...
                            mapForestBsfLevelsRecursive(levelNodes.flatMap(children), levelUpIndexTransform(index, levelNodes))
                }
            }
 ...
 */
fun <T, R, I> mapForestBfNodesIndexed(rootNodes: Iterable<T>,
                                      initialIndex: I,
                                      children: (T) -> Iterable<T>,
                                      nodeTransform: (index: I, node: T) -> R?,
                                      nextItemIndexTransform: (index: I, node: T) -> I,
                                      levelUpIndexTransform: (index: I, node: Iterable<T>) -> I): List<List<R>> =
        mapForestBfLevelsIndexed(rootNodes, initialIndex, children,
 ...
 */
fun <T, R, I> mapForestPreOrderIndexed(rootNodes: Iterable<T>,
                                       initialIndex: I,
                                       children: (T) -> Iterable<T>,
                                       ...
                                       postOrderIndexTransform: (traversalIndex: I, child: T, parent: T?) -> I = { i, _, _ -> i }): List<R> {
    ...
    fun mapTreeIndexedRecursive(originalParent: T?, originalItem: T, transformedParent: R?): R? {
        ...
        if (transformedItem != null) {
            val transformedChildren = children(originalItem).map { mapTreeIndexedRecursive(originalItem, it, transformedItem) }
 ...
 * -> post-order-indexing => postOrderIndexTransform = {it + 1}
 */
fun <T, R, I> mapForestPostOrderIndexed(rootNodes: Iterable<T>,
                                        initialIndex: I,
                                        children: (T) -> Iterable<T>,
...
fun <T, R, I> mapTreeIndexed(rootItem: T,
                             initialIndex: I,
                             children: (T) -> Iterable<T>,
                             transform: (item: T, preOrderIndex: I, children: List<R>, postOrderIndex: I) -> R?,
                             preOrderIndexTransform: (traversalIndex: I, child: T, parent: T?) -> I = { i, _, _ -> i },
                             postOrderIndexTransform: (traversalIndex: I, child: T, parent: T?) -> I = { i, _, _ -> i }): R {

    return mapForestPostOrderIndexed(listOf(rootItem), initialIndex, children, transform, preOrderIndexTransform, postOrderIndexTransform).single()
}
...
open class BaseForestStream<T>(val rootNodes: Iterable<T>) {
    ...
    open fun toTreeStream(children: (T) -> Iterable<T>): GenericTreeStream<T> = GenericTreeStream(rootNodes.single(), children)
...
open class GenericForestStream<T>(rootNodes: Iterable<T>, val children: (T) -> Iterable<T>) : BaseForestStream<T>(rootNodes) {


    /**
     * Pre order functions
     */
    open fun <R, I> mapForestPreOrderIndexed(initialIndex: I,
                                             ...
                                             postOrderIndex: (traversalIndex: I, child: T, parent: T?) -> I = { i, _, _ -> i }): BaseForestStream<R> =
            BaseForestStream(mapForestPreOrderIndexed(rootNodes, initialIndex, children, preOrderTransform, postOrderAction, preOrderIndex, postOrderIndex))

    fun <R> mapForestPreOrderIndexed(postOrderAction: (node: R, index: DfIndex, children: List<R>) -> Unit = { _, _, _ -> }, transform: (node: T, index: DfIndex, parent: R?) -> R?): BaseForestStream<R> =
            mapForestPreOrderIndexed(DfIndex(-1, 0, height(), -1), transform, postOrderAction,
                    preOrderIndex = { index, _, _ -> DfIndex(index.preOrderIndex + 1, index.postOrderIndex, index.level - 1, index.depth + 1) },
                    postOrderIndex = { index, _, _ -> DfIndex(index.preOrderIndex, index.postOrderIndex + 1, index.level + 1, index.depth - 1) })

    fun <R> mapForestPreOrder(postOrderAction: (node: R, children: List<R>) -> Unit = { _, _ -> }, transform: (node: T, parent: R?) -> R?): BaseForestStream<R> =
            mapForestPreOrderIndexed(0, {node, _, parent ->  transform(node, parent)}, {node, _, children ->  postOrderAction(node,children)})

    fun forEachPreOrder(action: (T) -> Unit) = mapForestPreOrder<Unit> { node, _ -> action(node) }
    fun forEachPreOrderIndexed(action: (preOrderIndex: DfIndex, T) -> Unit) = mapForestPreOrderIndexed<Unit> { node, index, _ -> action(index, node) }

    /**
     * Post order functions
     */
    open fun <R, I> mapForestPostOrderIndexed(initialIndex: I,
                                              ...
                                              postOrderIndex: (traversalIndex: I, childNode: T, parentNode: T?) -> I = { i, _, _ -> i }): BaseForestStream<R> =
            BaseForestStream(mapForestPostOrderIndexed(rootNodes, initialIndex, children, transform, preOrderIndex, postOrderIndex))
    ...
    fun forEachPostOrderIndexed(action: (preOrderIndex:DfIndex, postOrderIndex:DfIndex, T) -> Unit) =
        mapForestPostOrderIndexed<Unit> { node, preOrderIndex, _, postOrderIndex -> action(preOrderIndex, postOrderIndex, node) }
     ...
     */
    fun filterForestPostOrder(filteredNodeFactory: (node: T, children: List<T>) -> T, predicate: (node: T, children: List<T>) -> Boolean): GenericForestStream<T> =
            mapForestPostOrder<T> { node, children ->
                when (predicate(node, children)) {
                    false -> null //Predicate is not fulfilled, node is filtered out
                    true -> filteredNodeFactory(node, children) //Predicate is fulfilled, node is returned including children
                }
            }.toStream(children)

    fun filterForestPostOrder(filteredNodeFactory: (node: T, children: List<T>) -> T, predicate: (node: T) -> Boolean): GenericForestStream<T> = filterForestPostOrder(filteredNodeFactory, { node, _ -> predicate(node) })

    fun filterForestPreOrder(filteredNodeFactory: (node: T) -> T,
                             postOrderAction: (node: T, children: List<T>) -> Unit = { _, _ -> },
                             predicate: (node: T, parent: T?) -> Boolean): GenericForestStream<T> =
    ...
    fun filterForestPreOrder(filteredNodeFactory: (node: T) -> T,
                             predicate: (node: T, parent: T?) -> Boolean): GenericForestStream<T> = filterForestPreOrder(filteredNodeFactory, predicate = predicate)
     ...
     */
    fun filterForestByLeaves(filteredNodeFactory: (node: T, children: List<T>) -> T, predicate: (node: T) -> Boolean): GenericForestStream<T> =
            filterForestPostOrder(filteredNodeFactory, { node, children -> children.isEmpty() && predicate(node) })

    /**
     * Transform tree into flat list (BFS-traverse)
     * Input:
     *   a
     *  / |
     * b1  b2
     ...
     * listOf(a,b1,b2,c1)
     */
    fun mapForestBfs(): List<List<T>> = mapForestBfs { it }
    fun flatMapForestBfs(): List<T> = mapForestBfs().flatten()

    fun <R> mapForestBfs(transform: (node: T) -> R): List<List<R>> = mapForestBfsIndexed { _, node -> transform(node) }
    fun <R> flatMapForestBfs(transform: (node: T) -> R) = mapForestBfs(transform).flatten()

    fun <R> mapForestBfsIndexed(transform: (index:BfIndex, node: T) -> R): List<List<R>> =
    ...
    fun <R> flatMapForestBfsIndexed(transform: (index:BfIndex, node: T) -> R): List<R> = mapForestBfsIndexed(transform).flatten()

    fun <R> mapForestBfsLevelIndexed(transform: (depth: Int, levelNodes: Iterable<T>) -> List<R>): List<List<R>> =
            mapForestBfLevelsIndexed( rootNodes, 0, children, transform, { index, _ ->  index + 1} )
    fun <R> flatMapForestBfsLevelIndexed(transform: (depth: Int, levelNodes: Iterable<T>) -> List<R>): List<R> = mapForestBfsLevelIndexed(transform).flatten()

    fun forEachBfs(action: (T) -> Unit) = flatMapForestBfs{action(it)}
    fun forEachBfsIndexed(action: (index:BfIndex, T) -> Unit) = flatMapForestBfsIndexed(action)
    fun forEachLevelBfs(action: (Iterable<T>) -> Unit) = flatMapForestBfsLevelIndexed{_, levelNodes -> listOf(action(levelNodes))}
    fun forEachLevelBfsIndexed(action: (depth:Int, Iterable<T>) -> Unit) = flatMapForestBfsLevelIndexed{depth, levelNodes -> listOf(action(depth, levelNodes))}
     ...
     */
    fun nodeDepths(): List<Pair<T, Int>> = flatMapForestBfsIndexed { index, t -> t to index.depth }
...
open class BaseTreeStream<T>(val rootNode: T) {
    ...
    open fun toForestStream(children: (T) -> Iterable<T>): GenericForestStream<T> = GenericForestStream(listOf(rootNode), children)
    open fun toForestStream(): BaseForestStream<T> = BaseForestStream(listOf(rootNode))
}

open class GenericTreeStream<T>(rootNode: T, children: (T) -> Iterable<T>) : BaseTreeStream<T>(rootNode) {
    ...
    open fun <R, I> mapTreePostOrderIndexed(initialIndex: I,
                                            transform: (node: T, preOrderIndex: I, children: List<R>, postOrderIndex: I) -> R?,
                                            preOrderIndex: (traversalIndex: I, child: T, parent: T?) -> I = { i, _, _ -> i },
                                            postOrderIndex: (traversalIndex: I, child: T, parent: T?) -> I = { i, _, _ -> i }): BaseTreeStream<R> =
    ...
    fun <R> mapTreePostOrder(transform: (node: T, children: List<R>) -> R): BaseTreeStream<R> = forestStream.mapForestPostOrder(transform).toTreeStream()
     ...
     * listOf(a,b1,b2,c1)
     */
    fun flatMapTreeBfs(): List<T> = forestStream.flatMapForestBfs()
    fun <R> flatMapTreeBfs(transform: (node: T) -> R): List<R> = forestStream.flatMapForestBfs(transform)
    fun <R> flatMapTreeBfsIndexed(transform: (index: BfIndex, node: T) -> R): List<R> = forestStream.flatMapForestBfsIndexed(transform)
...
class TreeItemForestStream<T>(rootNodes: Iterable<TreeItem<T>>) : GenericForestStream<TreeItem<T>>(rootNodes, { it.children }) {
    constructor(rootItem: TreeItem<T>) : this(listOf(rootItem))

    fun <R, I> mapForestIndexed(initialIndex: I,
                                ...
                                postOrderIndex: (traversalIndex: I, child: TreeItem<T>, parent: TreeItem<T>?) -> I): TreeItemForestStream<R> =
            TreeItemForestStream(
                    super.mapForestPostOrderIndexed<TreeItem<R>, I>(initialIndex, { node, preOrderIndex, children, postOrderIndex -> transform(node, preOrderIndex, children, postOrderIndex).also { it.children.addAll(children) } },
                            preOrderIndex, postOrderIndex).rootNodes)
    ...
    fun <R> mapForestIndexedPreOrder(transform: (node: TreeItem<T>, preOrderIndex: Int, children: List<TreeItem<R>>) -> TreeItem<R>): TreeItemForestStream<R> =
            mapForestIndexed(-1, { node, preOrderIndex, children, _ -> transform(node, preOrderIndex, children) }, { i, _, _ -> i + 1 }, { i, _, _ -> i })
    ...
    fun <R> mapForestIndexedPostOrder(transform: (node: TreeItem<T>, preOrderIndex: Int, children: List<TreeItem<R>>) -> TreeItem<R>): TreeItemForestStream<R> =
            mapForestIndexed(0, { node, preOrderIndex, children, _ -> transform(node, preOrderIndex, children) }, { i, _, _ -> i }, { i, _, _ -> i + 1 })

    fun <R> mapForest(transform: (node: TreeItem<T>, children: List<TreeItem<R>>) -> TreeItem<R>): TreeItemForestStream<R> =
            mapForestIndexed(0, { node, _, children, _ -> transform(node, children) }, preOrderIndex = { i, _, _ -> i }, postOrderIndex = { i, _, _ -> i })
}
